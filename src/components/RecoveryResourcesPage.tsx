...
const RecoveryResourcesPage = () => {
  ...
  const mainCategories = [
    ...
    { id: "contractor", label: "Contractor", color: "bg-raspberry" },
    { id: "painter", label: "Painter", color: "bg-accent" },
    { id: "plumber", label: "Plumber", color: "bg-orange-500" },
    { id: "electrician", label: "Electrician", color: "bg-coral" },
    { id: "moving", label: "Moving", color: "bg-blue-500" },
  ...
  const handleSearch = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('search-recovery-resources', {
      ...
      if (data.cached && data.cachedAt) {
        ...
        toast({
          ...
          description: `Found ${data.resources.length} recovery services`,
        });
      }
      ...
      toast({
        title: "Search Error",
        description: "Failed to search for recovery services. Please try again.",
        variant: "destructive",
      });
      setResources([]);
    } finally {
  ...
  const filteredResources = resources
    .filter(resource => {
      ...
      if (resource.categories) {
        return resource.categories.includes(selectedCategory);
      }
    ...
    <div className="pb-20 min-h-screen bg-gradient-subtle">
      ...
      <div className="bg-gradient-primary text-primary-foreground p-4 pt-8">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold mb-1">Recovery Services</h1>
          </div>
        </div>
      </div>
          ...
          <Card className="shadow-soft border-yellow/30 bg-yellow/10">
            <CardContent className="p-6 text-center">
              ...
              <p className="text-muted-foreground mb-4">
                Access to recovery services requires a free account to ensure data privacy and prevent misuse.
              </p>
              <Button 
                onClick={() => navigate("/")}
        ...
        {user && !isGuest && (
          <>
            ...
            <div className="bg-background shadow-soft p-4">
              <p className="text-muted-foreground mb-4">
                Search Google business listings for disaster recovery services.
              </p>
            ...
            {showDisclaimer && (
              <div className="bg-muted/50 border border-border rounded-lg p-3 mb-4 relative">
                ...
                <p className="text-xs text-muted-foreground pr-8">
                  These listings come from Google Places. Please <strong>call to confirm</strong> they're available.
                </p>
              </div>
            )}
            ...
            <div className="space-y-4">
              {filteredResources.length === 0 && resources.length === 0 && !isSearching && (
                <div className="text-center py-8">
                  <p className="text-muted-foreground">Enter an address and press the *search* button for recovery services</p>
                </div>
              )}
              ...
              {filteredResources.map((resource, index) => (
                 <Card key={`${resource.source_id}-${resource.source}-${index}`} className="shadow-soft hover:shadow-medium transition-smooth">
                   <CardContent className="p-2">
                     ...
                     <p className="text-sm text-muted-foreground mb-1 leading-none">
...
export default RecoveryResourcesPage;