...
const RecoveryPage = () => {
  ...
  useEffect(() => {
    const loadProgress = async () => {
      if (user) {
        const ids = await getChecklistMirror('recovery', user.id);
        if (ids !== undefined) {
          ...
          await setChecklistMirror('recovery', user.id, taskIds);
          setCheckedItems(new Set(taskIds));
        }
      } else {
  ...
  const toggleCheck = async (taskUuid: string) => {
    ...
    if (user) {
      try {
        await writeChecklist({ type: 'recovery', userId: user.id, taskId: taskUuid, action: isCompleted ? 'check' : 'uncheck' });
      } catch {
